...
public class CalculatorUtils {
    ...
    public static Values getAverageValues(List<Values> values) {
        ...
        for (Values value :
            ...
            x2 += value.getX2();
            y2 += value.getY2();
            size1 += value.getSize1();
            size2 += value.getSize2();

        }
        return new Values(stressThickness / values.size(), curvature / values.size(),
                values.get(0).getTimestamp(), distance / values.size(),
                x1 / values.size(), y1 / values.size(), x2 / values.size(), y2 / values.size(),
                size1 / values.size(), size2 / values.size());
